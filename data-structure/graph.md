...
无向完全图：任意两顶点之间都存在边，$|E|=C_n^2=\frac{n(n-1)}{2}$

有向完全图：任意两顶点之间都存在方向相反的两条弧，$|E|=2C_n^2=n(n-1)$
...
- 无向图中顶点 v 的边条数，全部顶点的度之和=2*边数=2|E|
- 有向图中顶点 v 的弧条数，有向图中，顶点 v 的度=v 的入度+v 的出度，全部顶点v 的入度之和=全部出度之和=边数
...
- 有向图邻接表表示中，顶点的出度=顶点邻接边表的结点数，求顶点入度需要遍历全部结点的邻接边表
...
- 最小生成树（树形）不唯一，当图中各边权值互不相等时，最小生成树唯一
- 最小生成树的边的权值之和唯一
- 最小生成树的边数 = 顶点数 - 1
...
算法思想：初始每个每次选出权值最小的边，且边满足其两个顶点所在集合不相交，合并顶点集合，直到全部顶点加入

![image-20210719043726017](../assets/image-20210719043726017.png)

借助数据结构：[不相交集](data-structure/disjoint-set)、[优先队列](data-structure/priority-queue-heap)
...
算法思想：类似 Prim 算法，源顶点s；对每个顶点 u，遍历其邻接边顶点 v，若 v 经过中间顶点 u 到源顶点 s 路径长度 < 当前 v 到源顶点 s 路径长度 $u.d+w(u,v)<v.d$，则更新顶点 v 到 s 的最短路径 $v.d=u.d+w(u,v)$，v 的前驱顶点 $v.path=u$
...
$D^{(k)}:$ 从原始邻接边矩阵 $D^{(0)}$ 开始，每个顶点 i 到第 j 个顶点间有一条最短路径，且路径中的每个中间顶点编号不大于 k 。

路径：$v_i,\{编号\le k\ 的若干中间顶点\},v_j$

以中间顶点 $v_k$划分：$v_i,\{编号\le k-1\ 的若干中间顶点\},v_k,\{编号\le k-1\ 的若干中间顶点\},v_j$

![image-20210720200849909](../assets/image-20210720200849909.png)

递推式：$d_{ij}^0=w_{ij},\ k\ge 1, d_{ij}^k=min\{d_{ij}^{k-1},d_{ik}^{k-1}+d_{kj}^{k-1}\}$
...
TopSort 算法思想：从 AOV 网中选择一个没有前驱的顶点并输出，删除该顶点及临接边；重复执行，直到 AOV 网为空或不存在无前驱的顶点（存在环）

TopSort 算法实现（使用栈或队列）：

- 暂存入度0的顶点

- 若队列非空，出队顶点 v，将与其邻接的所有顶点入度-1；若其中有顶点入度为0，就将其入队；
...
>
> 关键路径：从源点到汇点的所有路径中，具有最大路径长度的路径；关键路径上的事件活动完成时，所有活动均已完成
>
> 关键活动：关键路径上的活动，其影响整个工程的完成时间。$a_i:d(i)=l(i)-e(i)=0$
>
> 最短完成时间：关键路径长度，即关键路径上各活动开销总和
...
关键路径可通过求 ve() 得出：从 ve(n) 到 ve(1) 逆序观察经 max 挑选出的均为关键路径各顶点
...
- 出栈一个出度为0的顶点 $v_j$，计算它所有临边前驱顶点 $v_k$的最迟(晚)发生（开始）时间：
  ...
  若 `vl[j]-w(vk,vj)<ve[k]`，则  `ve[k]=vl[j]-w(vk,vj)`
