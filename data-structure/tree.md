...
路径：（自上而下）连接两结点之间经过的结点序列；路径长度：路径上边的个数

结点的深度：从根节点到当前结点 k 的 路径/结点/层 数

结点的高度：从当前结点 k 到叶子结点的最大 路径/结点/层 数
...
- N 个结点的树共有 N-1 条边/分支，所有结点度数之和 N-1（根节点没有与父节点）

  总结点数=总分支数+1=结点总度数+1

- 度为 m 的树第 i 层结点数至多 $m^{i-1}$ 个

- 高度为 h 的树 m 叉树结点数至多 $1+m+...+m^{h-1}=\frac{1-m^h}{1-m}$ 个

- N 个结点的 m 叉树最小高度 $h=\lceil log_m(N(m-1)+1)\rceil$
...
- 对完全二叉树按层序编号：
  ...
  - 结点 i 所在层次（深度）$h_i=\lfloor log_2i\rfloor+1$
...
- 非空二叉树，**叶结点数**=双分支结点数+1，度0结点数=度2结点数+1，$n_0=n_2+1$
- **空指针数** = 2* 叶结点数+单分支结点数=2*度0结点数+度2结点数，$2n_0+n_1=n_0+n_1+n_2+1=n+1$
- 非空二叉树，第 k 层结点数至多 $2^{k-1}$
- 高度为 h 的二叉树结点数至多 $2^h-1$
...
- 一般二叉树：空节点补充关键字0，使整体对应完全二叉树（空间利用率低）
...
如：计算磁盘文件区块数（m叉树），计算二叉树深度 $h(T)=max\{h(T\rightarrow l),h(T\rightarrow r)\}+1$
...
递归栈的深度=树的深度，最坏情况下，二叉树是n结点、深度n的单支树
...
已知先序、中序序列，可以唯一确定一个二叉树

已知中序、后序序列，可以唯一确定一个二叉树

已知中序、层序序列，可以唯一确定一个二叉树

已知先序、后序序列，不能唯一确定一个二叉树
...
- 左孩子为叶结点的后继指向其父节点，右孩子为叶结点的前驱指向其父节点，
...
先将每个树用孩子兄弟表示法转化为二叉树，每个树对应的二叉树右子树为空；将第二个树作为第一个树的右子树，以此类推
...
- 若节点数N>0，则树由根 root 及 0个或1个，或2个左右子树组成
  ...
  若左子树非空，max{ 左子树结点值 } < 根结点值

  若右子树非空，根结点值 < min{ 左子树结点值 }
...
- LL 向右单旋转：左孩子右上旋转，结点A右下旋转，左孩子的右子树作为结点A的左子树
...
- RR 向左单旋转：右孩子左上旋转，结点A左下旋转，右孩子的左子树作为结点A的右子树

  ![image-20210716041204060](../assets/image-20210716041204060.png)

- LR 先左后右双旋转：左孩子B的右子树根结点C先RR左上旋转代替左孩子B，结点C再 LL 右上旋转代替A
...
- RL 先右后左双旋转：右孩子B的左子树根结点C先LL右上旋转代替右孩子B，结点C再 RR 左上旋转代替A
...
2. 构造新节点，从 F 中选两棵根结点权值最小的树作为其左、右子树，其权值=左子树根权值+右子树根权值；
3. 从 F 中删除那两棵子树，将新树结点加入 F；执行 2,3，直到 F 只剩一棵树

**特点**：

- 每个初始结点最终成为叶结点，权值越小的结点到根结点路径长度越长
- 构造过程共新建了 n-1 个双分支结点，huffman 树结点总数=2n-1
- huffman 树不存在度1结点
...
- 构造过程共新建了 $n_k=(n-1)/(k-1)$ 个 k 分支结点

- 判断需添加的虚空结点数：若 $n_k=(n_0-1)\%(k-1)=0$，说明初始结点足够形成 k-huffman
