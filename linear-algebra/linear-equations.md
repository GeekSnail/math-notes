...
![image-20210623185716958](../assets/image-20210623185716958.png)
...
![image-20210623185418494](../assets/image-20210623185418494.png)
...
> **定理 解的存在与唯一性**
>
> 线性方程组相容 $\\\Leftrightarrow$​ 增广矩阵最右列不是主元列 $\Leftrightarrow$​ 行阶梯型没有形如 $\begin{bmatrix}0 & ... & 0 & b\end{bmatrix}\ b\ne 0$​ 的行
...
向量 $\pmb{u}=c_1\pmb{v_1}+...+c_k\pmb{v_k}$  称为向量 $\pmb{v_1},\pmb{v_2},...,\pmb{v_k}$  以 $c_1, c_2, ...,c_k$  为权的线性组合

> 向量方程 $x_1\pmb{a_1}+x_2\pmb{a_2}+...+x_n\pmb{a_k}=\pmb{b}$ 与增广矩阵 $[a_1\ a_2\ ...\ a_n\ b]$ 的线性方程组有相同解集
>
> $\pmb{b}$ 可由 $\pmb{a_1},\pmb{a_2},...,\pmb{a_n}$线性表示 $\Leftrightarrow$ 对应增广矩阵的线性方程组有解

> **定义** $Span\left\{v_1, v_2, ... , v_k\right\}$：$R^n$ 中的向量 $v_1, v_2, ... , v_k$ 的所有线性组合 $c_1\pmb{v_1}+...+c_k\pmb{v_k}$ 的集合，称为由 $v_1, v_2, ... , v_k$ 张成的 $R^n$ 的子集（子空间）

$\pmb{b}\in Span\left\{v_1, v_2, ... , v_k\right\} \Leftrightarrow x_1\pmb{v_1}+x_2\pmb{v_2}+...+x_n\pmb{v_k}=\pmb{b}$ 有解
...
>
> 矩阵方程 $Ax=b$ 与向量方程 $x_1a_1+x_2a_2+...+x_na_k=\pmb{b}$ 有相同解集，
>
...
$A_{3\times 3}x=0,\ x$ 有 1 个自由变量（行阶梯型有 2 个非零方程）

$\pmb{x}=\begin{bmatrix}x_1\\x_2\\x_3\end{bmatrix}
=x_3 \begin{bmatrix}-a'_{12}\\-a'_{23}\\0\end{bmatrix}
=x_3\pmb{v}\ (x_3\in R)\Leftrightarrow\pmb{x}\in Span\left\{v\right\}$

几何意义：解集是 $R^3$​​ 中的一条过原点的直线

![image-20210626192223143](../assets/image-20210626192223143.png)
...
=x_2\pmb{v_1}+x_3\pmb{v_2}\ (x_{2,3}\in R)\Leftrightarrow\pmb{x}\in Span\left\{v_1, v_2\right\}$

几何意义：解集是 $R^3$​ 中的一个过原点的平面

![image-20210626192347030](../assets/image-20210626192347030.png)

**参数向量形式(解集)**

$\pmb{x}=t_1\pmb{u}+t_2\pmb{v}\ (t_{1,2}\in R)$
...
$\pmb{x}=\begin{bmatrix}x_1\\x_2\\x_3\end{bmatrix}=x_3
  \begin{bmatrix}-a'_{12}\\-a'_{23}\\0\end{bmatrix}+
  \begin{bmatrix}b'_1\\b'_2\\0\end{bmatrix}=x_3\pmb{v}+\pmb{p}\ (x_3\in R)$

**参数向量形式(解集)**

$\pmb{x}=t\pmb{v}+\pmb{p}\ (t\in R)$

![image-20210626194648001](../assets/image-20210626194648001.png)

## 1.7 线性无关

$Ax=0 \Leftrightarrow x_1\pmb{a_1}+x_2\pmb{a_2}+x_3\pmb{a_3}=\pmb{0}$
...
>
> 一向量 $v$ 线性无关 $\Leftrightarrow \pmb{v}\ne\pmb{0}$
>
...
证明：$k>n\Rightarrow\exist$ 自由向(变)量 $\Leftrightarrow Ax=0$ 有非零解

> **定理** $R^n$ 中向量组 $S=\left\{v_1,...,v_k\right\}, \exist v_i=\pmb{0} \Rightarrow$向量组相关
> （零向量可由任意一组基线性表示）
...
- 投影变换：$A=\begin{bmatrix}
  1 & 0 & 0\\
  0 & 1 & 0\\
  0 & 0 & 0\end{bmatrix},\ T:R^3\rightarrow R^2$​ 把 $R^3$​ 的点投影到 $x_1x_2$​ 平面
...
- 伸缩变换：$r\in R,T:R^2\rightarrow R^2, T(x)=rx$​（此时 r 为线性变换 A 的特征值）
