...
每一个区设置相应的**权限域名服务器** (ANS)，用来保存该区中的所有主机的域名到 IP 地址的映射。

![image-20210902023239273](../assets/image-20210902023239273.png)

- 本地域名服务器：当一台主机发出 DNS 查询请求时，报文先发送给本地域名服务器
...
- 递归查询：作为DNS客户端请求1次，最终得到响应

  （中间DNS服务器根据响应继续查询，直到一个权限域名服务器查询成功再一级级返回）

- 迭代查询：作为DNS客户端逐级各请求1次

  本地域名服务器作为DNS客户端请求根NS，根NS根据顶级域名响应顶级NS地址 (dns.com)

  本地域名服务器向顶级NS请求，顶级NS响应权限NS地址 (dns.example.com)

  本地域名服务器向权限NS请求，若查询到则返回IP
...
- 主动 PORT 模式：客户端开临时端口M 监听，使用控制连接发送 `PORT M`，服务端在端口 20 与客户端端口 M 进行数据连接
- 被动 PASV 模式：客户端使用控制连接发送 `PASV`，服务端开临时端口并返回 (ip:port)；客户端以此打开数据连接
...
- SMTP 限于传送 7 位的 ASCII 码，不能传送二进制文件
...
- 持续连接：服务器在响应后仍在一段时间内保持该连接，该客户进程可以继续在该连接上传送后续的 HTTP 报文（HTTP/1.1）
  ...
  - 流水线：客户在收到 HTTP 响应报文之前能够接着发送新的请求报文。一连串请求需要一个 RTT 稍多一点点时间
