...
16-bit Identifier：每产生一个数据报，计数器加1，将值赋给标识字段；用于分片/重组

3-bit Flags：bit[0]=0，bit[1]: DF(Dont't Fragment) 0-允许分片, bit[2]: MF(More Fragment) 0-最后一片

13-bit Fragment-offset(8B)：相对于用户数据字段的起点，该片开始位置

Time-to-live(s)：允许保持的最大时间。每经过一个路由器，`TTL-=1`；当 `TTL=0`，表示不可达丢弃
...
网络号全0，主机号全0：本网络本主机，作为源地址

网络号全0，特定主机号：本网络特定主机，作为源地址

网络号全1，主机号全1：本网络广播地址，作为目的地址
...
> 路由器总是有多个IP地址，每个端口对应其直连的不同网络号地址
...
NAT地址后跟端口号：即一个主机进程LAN与WAN地址不同端口号的映射

NAT路由器转发IP数据报时，需要做LAN与WAN间的 [IP地址:port] 转换
...
- A 在ARP缓存查看是否有 B 的IP地址，若有，则将对应MAC地址写入MAC帧，再发出
 否则 A 或 B 刚入网不知道对方
- A 封装一个(目的IP: B) 的数据报，(目的MAC: FF-FF-FF-FF-FF-FF-FF) 的帧，广播ARP请求
- B 收到ARP请求，发现目的IP地址匹配，向 A 单播ARP响应（包含B的IP-MAC地址），并更新自己的 ARP 缓存
 其它主机发现不匹配则忽略
...
- DHCP discover：DHCP客户端封装发现报文：UDP-源端口68-目的端口67，IP数据报源IP地址0.0.0.0，目的地址255.255.255.255，广播
...
- RIP 报文被封装在 UDP(port-520) 报文中

- 更新计时器：RIP进程每隔 30 秒向每个相邻路由器广播(v1)或多播(v2)一个包含完整路由表的报文
...
- 每个路由器每30s与相邻路由器交换信息，n次后最多可知道n跳距离的网络
- RIP以最少跳数衡量距离，不管途径的网络质量
- 网络出现故障时，出现慢收敛（需长时间才将此消息传到区域所有路由器）——坏消息传得慢
...
- 初始化/链路状态变化时，使用**洪泛法**向自治系统所有路由器发信息
...
- OSPF 报文被封装在 IP(protocol-89) 数据报中，数据报短，无需分片，避免丢失重传
...
- 采用路径-向量路由选择，力求寻找一条能到达目的网络的较好路由（非最佳）
