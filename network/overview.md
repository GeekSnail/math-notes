...
- 分组交换网络：也称包交换网络。将数据分成较短长度的数据块，对每个数据块封装报文再存储-转发
...
- 排队时延：分组在路由器输入队列等待处理、输出队列等待转发的时间
...
协议：控制两个对等实体进行通信的规则集合。如：请求/传播/响应的规则

- 语法：数据与控制信息的结构/格式。如：交换的信息格式
- 语义：需要发出何种控制信息，完成何种动作/响应。如：数据何时必须重传或丢弃
- 同步：动作实现的时序规定。
...
- 可靠/不可靠服务，可靠：纠错/检错/应答机制，正确送到目的地。不可靠：可靠性由应用或用户来保障
...
![image-20210818070157652](../assets/image-20210818070157652.png)
