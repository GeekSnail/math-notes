...
机器数据 01234567H 小端存放：67H 45H 23H 01H（指令代码包含存储顺序）
...
数符S 1位，阶码(移码)E 8位，尾数(原码)M 23位，移码偏置127=7FH

规格化的短浮点数真值：$(-1)^S\times 1.M\times 2^{E-127}, E\in [1,254]$​​，有效 $e\in[-126,127]$​ 

绝对值最小值：E=1, M=0, $N=1.0\times 2^{1-127}$​​​

绝对值最大值：E=254, M=.11...11, $N=1.11...11\times 2^{254-127}=2^{127}\times (2-2^{-23})$
...
阶码移码，尾数原码（最高位1隐藏），加减运算与基于补码表示的浮点数类似，
...
> 无符号数依据 CF 标志判断进位/借位/溢出，符号数依据 OF 标志判断溢出
...
对无符号数，7 + 7 = 0111 + 0111 = 1110，CF=Cout=0，最高位无进位，相同符号数相加结果符号相反—溢出
