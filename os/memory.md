...
- 首次适应FF：空闲分区按地址递增次序链接，顺序查找第一个满足大小的分区
...
- 最佳(小)适应BF：空闲分区按容量递增次序链接，顺序查找第一个满足大小的分区

  缺点：分配”最佳“分区，导致留下更小的难以利用的分区，产生更多的外部碎片
...
### 请求分页管理
初始化时，根据虚拟页是否在内存中，设置 `_PAGE_BIT_PRESENT`，`pte_set_flags(pte, _PAGE_PRESENT)`
#### 页表
...
淘汰页：最近最久未使用的页，页面引用次数 `_count` 最少的页（向前看）
...
也称：最近未使用 NRU。页面设置一个最近访问位 `used`

数据结构：循环链队列（初始指针 `p = p->next`）

1. 从当前位置 `p` 开始遍历链表，若页面存在，置访问位 `p->used = 1`，返回

2. 若存在空闲页帧，页指针入队并置访问位 `p = newp, p->used = 1`，更新指针 `p = p->next`，返回

3. 否则进行淘汰替换：循环遍历链表，

   若页面访问位0，`p = newp, p->used = 1`，更新指针 `p = p->next`，退出循环
...
1. 从当前位置 `p` 开始遍历链表，若 `p->used == 0 && p->mod == 0`，`p = newp`，更新指针 `p = p->next`，返回

2. 从当前位置 `p` 开始遍历链表，若 `p->used == 0 && p->mod == 1`，`p = newp`，更新指针 `p = p->next`，返回

   否则即为已访问页，重置访问位 `p->used == 0`
...
进程抖动：进程缺页率高，频繁的页面调度，调页时间多于进程执行时间。

如：FIFO页面置换算法

原因：CPU调度程序为提高CPU利用率，增加进程多道程度
